...
def qlearn_fa(simenv, q, gamma, epsilon, alpha, num_episodes, max_episode_len, 
    ...
    for episode in range(num_episodes):
        ...
        while episode_length < max_episode_len:
            ...
            ### target
            with torch.no_grad():
                if term_status:     ### terminate state q(St,At)=0
                    target = torch.tensor(reward)
                
                else:               ### Semi-gradient Q-learning
                    q_s_1=q.forward(next_state)[torch.argmax(q.forward(next_state))]
                    target = reward+gamma*q_s_1
...
def qlearn_fa_replay(simenv, q, gamma, epsilon, alpha, num_episodes, max_episode_len, 
    ...
    for episode in range(num_episodes):
        ...
        while episode_length < max_episode_len:
            ...
            for re in replay:
                (St,At,Re_reward,St_n,REterm_status) = re
            ...
            ### target
                with torch.no_grad():
                    if REterm_status:     ### terminate state q(St,At)=0
                        target = torch.tensor(Re_reward)
                
                    else:               ### Semi-gradient Q-learning
                        q_s_1=q.forward(St_n)[torch.argmax(q.forward(St_n))]
