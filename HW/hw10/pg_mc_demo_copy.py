...
from pg_mc_copy import pg_mc
import gym
import torch
from torch.distributions import Categorical
torch.manual_seed(1234)  #let's make things repeatable!
show_plots=True
...
policy=MLP(simenv.numFeatures,simenv.numActions)        #### 4 features with 2 actions
    
#Compute policy using Policy Gradient with Monte Carlo (REINFORCE)
pg_mc(simenv,policy,0.99,2e-5,25000,500,showPlots=show_plots)
...
torch.manual_seed(123)  #let's make things repeatable!
                        #A question to consider: What happens if we change this seed value; i.e., the initial conditions
                        #                        on our neural network weights? Does this affect convergence?
...
#Compute policy using Policy Gradient with Monte Carlo (REINFORCE)
pg_mc(simenv,policy,0.99,1e-4,2000,500,showPlots=show_plots)
