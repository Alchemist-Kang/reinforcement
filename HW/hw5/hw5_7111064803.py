...
def q_st_at(cat_and_mouse_env,init_st, num_episodes, max_iteration, gamma, alpha):
    ...
    for num_episode in range(num_episodes):
        ...
        while length < max_iteration:
            ...
            q[current_state,Action_t]=q[current_state,Action_t]+alpha*(r+gamma*np.max(q[st_n])-q[current_state,Action_t])
...
(policy,q) = q_st_at(cat_and_mouse_env=cm,init_st=cm.currentState(),max_iteration=100,num_episodes=500,gamma=0.9,alpha=0.1)
...
(policy_2,q_2) = q_st_at(cat_and_mouse_env=cm_2,init_st=cm_2.currentState(),max_iteration=250,num_episodes=1800,gamma=0.9,alpha=0.4)
